...
class MultiPerspective(Layer):
    ...
    def build(self, input_shape):
        ...
        # print(self.mp_dim,embedding_size*2*self.strategy)
        ...
        self.kernel_fw = self.kernel[:, : embedding_size]
        self.kernel_bw = self.kernel[:, embedding_size: embedding_size * 2]
    ...
    def call(self, inputs):
        ...
        if self.matching_type == "fullmatching":
            # full matching ops
            matching_fw = self._full_matching(h1_fw, h2_fw, self.kernel_fw)
            matching_bw = self._full_matching(h1_bw, h2_bw, self.kernel_bw)
        ...
        if self.matching_type == "avgattentivematching":
            # attentive matching ops
            matching_fw = self._attentive_matching(
                h1_fw, h2_fw, cosine_matrix_fw, self.kernel_fw)
            matching_bw = self._attentive_matching(
                h1_bw, h2_bw, cosine_matrix_bw, self.kernel_bw)
            list_matching.extend([matching_fw, matching_bw])

        if self.matching_type == "maxattentivematching":
        ...
        if self.matching_type == "maxpoolingmatching":
            # max pooling matching ops
            matching_fw = self._max_pooling_matching(h1_fw, h2_fw, self.kernel_fw)
            matching_bw = self._max_pooling_matching(h1_bw, h2_bw, self.kernel_bw)
            list_matching.extend([matching_fw, matching_bw])
