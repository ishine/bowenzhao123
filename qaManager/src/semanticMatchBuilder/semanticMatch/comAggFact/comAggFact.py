...
class ComparedAggFactor(object):
    ...
    def build(self):
        ...
        mergedVectorB = concatenate([selfAttLSTM1SenB, Att_LSTM1SenB], axis=-1, name='mergedVectorB')
            ...
            #vectorA = minus(mergedVectorA)
            #vectorB = minus(mergedVectorB)
            #mergedVectorA = concatenate([mergedVectorA, vectorA], axis=-1, name='diffconcatLayerA')
            #mergedVectorB = concatenate([mergedVectorB, vectorB], axis=-1, name='diffconcatLayerB')

        #if self.merge_type == "mul":  # 只加入相乘后的向量
            #mutiply = Lambda(self.pairwisemul)
            #vectorA = mutiply(mergedVectorA)
            #vectorB = mutiply(mergedVectorB)
            #mergedVectorA = concatenate([mergedVectorA, vectorA], axis=-1, name='mulconcatLayerA')
            #mergedVectorB = concatenate([mergedVectorB, vectorB], axis=-1, name='mulconcatLayerA')

        #if self.merge_type == "complex":  # 同时加入相减，相乘后的向量
            #minus = Lambda(self.absminus)
            #vector1A = minus(mergedVectorA)
            #vector1B = minus(mergedVectorB)

            #mutiply = Lambda(self.pairwisemul)
            #vector2A = mutiply(mergedVectorA)
            #vector2B = mutiply(mergedVectorB)

            #mergedVectorA = concatenate([mergedVectorA, vector1A, vector2A], axis=-1, name='complexconcatLayerA')
            #mergedVectorB = concatenate([mergedVectorB, vector1B, vector2B], axis=-1, name='complexconcatLayerB')
